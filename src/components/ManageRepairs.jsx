...
export const ManageRepairs = () => {
  ...
  const fetchRepairItems = async () => {
    try {
      console.log('Fetching repair items...');
      const response = await axios.get(ENDPOINTS.REPAIR_REQUESTS.UNDER_REPAIR, {
        headers: getAuthHeader()
      });
      console.log('Repair items response:', response.data);
      setRepairItems(response.data);
      setError(null);
    } catch (err) {
      console.error('Error fetching repair items:', err);
      setError(err.response?.data?.message || 'Failed to fetch repair items');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleStatusUpdate = async (itemId, newStatus) => {
    try {
      ...
      await axios.patch(
        ENDPOINTS.REPAIR_REQUESTS.UPDATE_UNDER_REPAIR(itemId),
        { 
          status: newStatus,
          remarks: remark 
        },
  ...
  const handleDownload = async () => {
    try {
      ...
      a.href = url;
      a.download = `repair-item-photo-${Date.now()}.jpg`;
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-6">Manage Repairs</h1>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {repairItems.map((item) => (
            <div key={item._id} className="bg-white rounded-lg shadow-md p-4">
              <div className="mb-4">
                <h3 className="font-semibold">Location: {item.location}</h3>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-40">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            <h2 className="text-xl font-semibold mb-4">
              {action === 'in_progress' ? 'Start Repair' : 'Complete Repair'}
            </h2>
            <div className="mb-4">
            ...
            <div className="flex justify-end space-x-4">
              ...
              <button
                onClick={() => handleStatusUpdate(selectedItem._id, action)}
              ...
              >
                {action === 'in_progress' ? 'Start Repair' : 'Mark Complete'}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className={`bg-white rounded-lg p-4 ${isFullscreen ? 'fixed inset-4' : 'max-w-3xl w-full'}`}>
            <div className="flex justify-between items-center mb-2">
              <h3 className="text-lg font-semibold">Repair Item Photo</h3>
