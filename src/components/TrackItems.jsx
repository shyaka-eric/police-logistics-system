...
export const TrackItems = () => {
  ...
  const fetchInUseItems = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('http://localhost:5000/api/stock/issued', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const filteredItems = items.filter(item => {
    ...
    // For UnitLeader, never show completed items regardless of filter
    if (userRole === 'UnitLeader' && item.status === 'completed') {
      return false;
    }
    ...
    <div className="max-w-7xl mx-auto p-6">
      ...
      <div className="mb-6 flex flex-wrap gap-4">
        ...
        >
          ...
          {userRole !== 'UnitLeader' && <option value="completed">Completed</option>}
              ...
              {filteredItems.length === 0 ? (
                ...
                filteredItems.map((item) => (
                  <tr key={item._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">{item.purpose}</td>
                  </tr>
                ))
              )}
            </tbody>
